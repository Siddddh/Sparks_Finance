...
const MODULE_KEYS = [{ id: 'stocks', label: 'Trading' }, { id: 'loans', label: 'Loans & Notes' }, { id: 'assets', label: 'Assets' }, { id: 'financials', label: 'Financial Statements' }];   // for module-access emails
...
const RBAC_MODULES = ['stocks', 'loans', 'assets', 'financials'];                // keep in sync with the client MODULES registry
    ...
    try {
      ...
      const users = list.users.map(u => {
        ...
        return {
          ...
          perms: owner ? cleanPerms({ stocks: 'delete', loans: 'delete', assets: 'delete', financials: 'delete' }) : cleanPerms(claims.perms || doc.perms),
...
exports.createOrg = onRequest({ region: 'us-central1', cors: true }, async (req, res) => {
  ...
  try {
    ...
    await setMembership(ref.id, tok.uid, tok.email, 'owner', { stocks: 'delete', loans: 'delete', assets: 'delete', financials: 'delete' }, name, { joinedAt: Date.now(), invitedBy: tok.uid });
...
exports.ensurePersonalOrg = onRequest({ region: 'us-central1', cors: true }, async (req, res) => {
  ...
  try {
    ...
    await setCompanyMembership(ref.id, coRef.id, tok.uid, tok.email, 'admin', { stocks: 'delete', loans: 'delete', assets: 'delete', financials: 'delete' }, 'Personal', 'Personal', { joinedAt: Date.now() });
...
exports.createCompany = onRequest({ region: 'us-central1', cors: true }, async (req, res) => {
  ...
  try {
    ...
    if (!isOwnerEmail((auth.tok.email || '').toLowerCase())) {
      try { await setCompanyMembership(orgId, ref.id, auth.tok.uid, (auth.tok.email || '').toLowerCase(), 'admin', { stocks: 'delete', loans: 'delete', assets: 'delete', financials: 'delete' }, await _orgName(orgId), name, { joinedAt: Date.now() }); } catch (e) {}
    }
